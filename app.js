...
function startCamera() {
  ...
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    showCamWarning('Camera API not supported in this browser. Please use Chrome or Safari on HTTPS.');
    return;
  }
  stopCamera(false);
    ...
    .then(function(stream) {
      ...
      video.onloadedmetadata = function() {
        video.play().then(function() {
          ...
          showCamWarning('Could not start video: ' + err.message);
        });
      };
    })
    .catch(function(err) {
      handleCameraError(err, 'camWarning', 'camWarningMsg');
    });
}
...
// ════════════════════════════════════════════════════════════════
// VISITOR CAMERA — ENTRY MODAL
// ════════════════════════════════════════════════════════════════
function startVisitorCamera() {
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    showToast('Camera not supported on this device/browser', 'error'); return;
  }
  stopVisitorCamera(false);
  var constraints = { video: { facingMode: { ideal: 'user' }, width: { ideal: 1280 }, height: { ideal: 720 } }, audio: false };
  navigator.mediaDevices.getUserMedia(constraints)
    .then(function(stream) {
      vcamStream = stream;
...
function clearVisitorPhoto() {
  ...
  var vf = document.getElementById('v-photo-file');
  if (vf) vf.value = '';
}

// ════════════════════════════════════════════════════════════════
// BOOT APP — ROLE-BASED NAV
// ════════════════════════════════════════════════════════════════
function bootApp() {
  ...
  nav.innerHTML =
    ...
    '<a class="nav-link" data-pane="pane-registry" data-dir="IN" onclick="loadRegistry(\'IN\'); navToPane(\'pane-registry\')">' +
      '<i class="fas fa-sign-in-alt" style="color:var(--success)"></i> <span>Inbound (IN)</span></a>' +
    '<a class="nav-link" data-pane="pane-registry" data-dir="OUT" onclick="loadRegistry(\'OUT\'); navToPane(\'pane-registry\')">' +
...
function renderRegistry() {
  ...
  rows.forEach(function(p) {
    ...
    tr.innerHTML = chkHtml +
      ...
      '<td><span style="font-size:11px">' + esc(p.CreatedTime||'') + '</span></td>' +
...
function openVisitorEntryModal() {
  ...
  visitorEntryTimeInterval = setInterval(function() {
    ...
    if (el && document.getElementById('visitorEntryModal').classList.contains('show')) {
      ...
      clearInterval(visitorEntryTimeInterval);
      visitorEntryTimeInterval = null;
    }
  }, 1000);
}
...
function submitVisitorEntry() {
  ...
  google.script.run
    ...
    .withFailureHandler(function(err) {
      hideLoader();
      showToast('Connection error: ' + (err.toString()), 'error');
    })
    .addVisitorRowToDatabase(visitorData);
}
...
function lookupVisitorForExit() {
  ...
  for (var i = 0; i < VISITOR_REGISTRY.length; i++) {
    if ((VISITOR_REGISTRY[i].PassNo || '').toUpperCase() === passNo) {
      vp = VISITOR_REGISTRY[i];
      break;
    }
  }
  ...
  if (!vp) {
    resultDiv.innerHTML =
      ...
      '<strong style="color:var(--danger)">Visitor Pass Not Found</strong><br>' +
      '<span style="font-size:12px;color:var(--text-2)">No record for: <code>' + esc(raw) + '</code></span></div>';
    return;
  }

  if ((vp.Status || '').toUpperCase() === 'OUT') {
    resultDiv.innerHTML =
      '<div class="p-3 border border-warning rounded" style="background:var(--warning-bg)">' +
      '<i class="fas fa-exclamation-triangle" style="color:var(--warning)"></i> ' +
      '<strong style="color:var(--warning)">Visitor Already Exited</strong><br>' +
  ...
  document.getElementById('exitStep2').innerHTML =
    ...
    '<div><div style="font-size:1rem;font-weight:700;color:var(--text)">' + esc(vp.VisitorName) + '</div>' +
    ...
    '<tr><td>Phone</td><td>' + esc(vp.ContactNo||'—') + '</td></tr>' +
  ...
  google.script.run
    .withSuccessHandler(function(res) {
      ...
      if (res.success) {
        ...
        var vIdx = -1;
        for (var i = 0; i < VISITOR_REGISTRY.length; i++) {
          if (VISITOR_REGISTRY[i].PassNo === passNo) { vIdx = i; break; }
        }
        if (vIdx !== -1) {
          VISITOR_REGISTRY[vIdx].Status = 'OUT';
          VISITOR_REGISTRY[vIdx].OutTime = outTimeISO;
        }
        renderVisitorRegistry();
    ...
    .withFailureHandler(function(err) {
      hideLoader();
      showToast('Connection error: ' + (err.message || err.toString()), 'error');
    })
    .recordVisitorExit({ passNo: passNo, outTime: outTimeISO, username: SESSION ? SESSION.username : 'Security' });
}
...
function openVisitorDetail(passNo) {
  ...
  document.getElementById('visitorDetailBody').innerHTML =
    ...
    '<div class="text-end">' +
    (isIn
      ? '<span class="pill" style="font-size:11px;background:rgba(255,255,255,0.2);border:1px solid rgba(255,255,255,0.4);color:#fff"><i class="fas fa-circle" style="font-size:7px"></i> INSIDE</span>'
    ...
    '<div class="col-lg-6">' +
    '<div style="font-size:11px;color:#059669;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid #d1fae5;padding-bottom:6px;margin-bottom:12px">Visitor Information</div>' +
    '<table class="detail-table">' +
    ...
    '</table></div>' +
    '<div class="col-lg-6">' +
    '<div style="font-size:11px;color:#059669;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid #d1fae5;padding-bottom:6px;margin-bottom:12px">Time & Duration</div>' +
    '<table class="detail-table">' +
    ...
    (v.PhotoUrl
      ? '<img src="' + v.PhotoUrl + '" style="width:100%;max-height:160px;object-fit:contain;border-radius:10px;border:1px solid var(--border)" onerror="this.outerHTML=\'<div class=\\\'p-3 text-center text-muted bg-light border rounded\\\' style=\\\'font-size:12px\\\'><i class=\\\'far fa-image fa-2x d-block mb-1\\\'></i>Not available</div>\'">'
...
function printVisitorPass(v) {
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
    ...
    '<title>Visitor Gate Pass - ' + esc(v.PassNo) + '</title>' +
    ...
    'h2{margin:0 0 3px;font-size:1.1rem}' +
    '.pno{background:rgba(255,255,255,0.2);border-radius:4px;padding:3px 8px;font-family:monospace;font-size:10px;letter-spacing:0.15em;display:inline-block;margin-top:5px}' +
    'table{width:100%;border-collapse:collapse;margin-bottom:14px}' +
    'td{padding:6px 8px;border-bottom:1px solid #e2e8f0;font-size:12px}' +
    'td:first-child{font-weight:bold;color:#64748b;width:140px}' +
    ...
    '.dur{font-family:monospace;font-size:1rem;font-weight:700;color:#0f4c81;background:#e8f1fb;padding:3px 10px;border-radius:5px;display:inline-block}' +
    '.s-in{background:#d1fae5;color:#065f46;padding:2px 8px;border-radius:4px;font-weight:800;font-size:10px}' +
    '.s-out{background:#dbeafe;color:#1e40af;padding:2px 8px;border-radius:4px;font-weight:800;font-size:10px}' +
    ...
    '<div><h2>Transmech LLC — Visitor Gate Pass</h2>' +
    '<div style="font-size:10px;opacity:0.75">Industrial Area 3, Jebel Ali, Dubai, UAE</div>' +
    '<div class="pno">' + esc(v.PassNo) + '</div></div></div>' +
    ...
    '<table>' +
    '<tr><td>Status</td><td>' + ((v.Status||'IN').toUpperCase()==='IN' ? '<span class="s-in">INSIDE</span>' : '<span class="s-out">EXITED</span>') + '</td></tr>' +
    '<tr><td>In Time</td><td>' + formatDateTime(v.InTime) + '</td></tr>' +
    '<tr><td>Out Time</td><td>' + (v.OutTime ? formatDateTime(v.OutTime) : '—') + '</td></tr>' +
    '<tr><td>Duration</td><td>' + (v.OutTime ? '<span class="dur">' + duration + '</span>' : '—') + '</td></tr>' +
    '</table>' +
...
function openPassDetail(passNo) {
  ...
  document.getElementById('passModalBody').innerHTML =
    ...
    '<table class="detail-table">' +
    '<tr><td>Date / Time</td><td>' + esc(p.CreatedTime||'—') + '</td></tr>' +
...
function printPass(p) {
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Gate Pass - ' + esc(p.PassNo) + '</title>' +
    '<style>body{font-family:Arial,sans-serif;padding:20px;max-width:560px;margin:auto}' +
    'h2{color:#0f4c81}table{width:100%;border-collapse:collapse}' +
    'td{padding:6px 8px;border-bottom:1px solid #eee;font-size:12px}' +
    'td:first-child{font-weight:bold;color:#666;width:150px}' +
    '.status{display:inline-block;padding:2px 8px;border-radius:4px;font-weight:bold;font-size:11px}' +
    '.confirmed{background:#d1fae5;color:#059669}.pending{background:#fef3c7;color:#d97706}.rejected{background:#fee2e2;color:#dc2626}' +
    '@media print{button{display:none}}</style></head><body>' +
    '<h2>Transmech LLC — Gate Pass</h2>' +
    '<p style="color:#666;font-size:12px">Industrial Area 3, Jebel Ali, Dubai, UAE</p><hr>' +
    '<table>' +
    '<tr><td>Pass No</td><td><strong>' + esc(p.PassNo) + '</strong></td></tr>' +
    '<tr><td>Status</td><td><span class="status ' + (p.Status||'').toLowerCase() + '">' + esc(p.Status) + '</span></td></tr>' +
    '<tr><td>Date / Time</td><td>' + esc(p.CreatedTime||'—') + '</td></tr>' +
    ...
    '<tr><td>Approved By</td><td>' + esc(p.ApprovedBy||'—') + '</td></tr>' +
    '</table>' +
    '<p style="margin-top:24px;font-size:10px;color:#999">Printed: ' + new Date().toLocaleString() + '</p>' +
    '<script>window.onload=function(){window.print();}<\/script>' +
    '</body></html>';
  try {
    var win = window.open('', '_blank');
...
function processWorkflow(passNo, type) {
  ...
  google.script.run
    ...
    .withFailureHandler(function(err) { hideLoader(); showToast('Server error: ' + (err.message || err.toString()), 'error'); })
    .updateWorkflow({ passNo: passNo, type: type, username: SESSION.username });
}
...
function submitPass(e) {
  ...
  google.script.run
    ...
    .withFailureHandler(function(err) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> SUBMIT GATE PASS';
      showToast('Server Error: ' + (err.message || err.toString()), 'error');
    })
...
function doBulkDelete() {
  ...
  google.script.run
    ...
    .withFailureHandler(function(err) { hideLoader(); showToast('Server error: ' + (err.message || err.toString()), 'error'); })
    .deletePasses(selected, SESSION.role);
}
...
function doScan() {
  ...
  if (vp) {
    ...
    resultDiv.innerHTML =
      ...
      '<div style="font-size:13px"><strong>' + esc(vp.VisitorName) + '</strong> | ' + esc(vp.Company||'—') + ' | ' + esc(vp.GateNo||'Gate') + '<br>' +
  ...
  if (p) {
    ...
    resultDiv.innerHTML =
      ...
      '<div style="font-size:13px"><strong>' + esc(p.VisitorName) + '</strong> | ' + esc(p.PassType) + ' | ' + esc(p.GateNo||'Gate') + '<br>' +
      '<span class="text-muted">' + esc(p.CreatedTime||'') + '</span></div>' +
    ...
    resultDiv.innerHTML =
      '<div class="p-3 border border-danger rounded" style="background:var(--danger-bg)">' +
      '<i class="fas fa-times-circle text-danger me-2"></i>' +
      '<strong style="color:var(--danger)">Pass Not Found</strong><br>' +
      '<span style="font-size:12px;color:var(--text-2)">No record for: <code>' + esc(token) + '</code></span></div>';
  }
}
...
function runReport() {
  ...
  setTimeout(function() {
    ...
    records.forEach(function(x, i) {
      ...
      tr.innerHTML =
        '<td>' + (i+1) + '</td>' +
        '<td style="font-size:11px">' + esc(x.CreatedTime||'') + '</td>' +
...
function exportReportExcel() {
  ...
  try {
    ...
    var rows = records.map(function(x, idx) {
      return {
        'S.No': idx+1, 'Pass No': x.PassNo||'', 'Date / Time': x.CreatedTime||'',
        ...
        'Status': x.Status||'', 'Created By': x.CreatedBy||'', 'Approved By': x.ApprovedBy||''
      };
    });
